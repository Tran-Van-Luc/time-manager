...
import { parseFile, exportTemplate, ParsedRow, ParseResult } from './importHelpers';
...
export default function ImportDialog({ visible, onClose, onParsed }: Props) {
  ...
  const handleConfirm = async () => {
    ...
    try {
      ...
      const msg = e && e.message ? String(e.message) : 'Không thể đọc tệp Excel. Vui lòng kiểm tra định dạng.';
      ...
      <View style={{ flex:1, backgroundColor:'rgba(0,0,0,0.4)', justifyContent:'center', alignItems:'center' }}>
        <View style={{ width:680, maxWidth:'95%', backgroundColor:'#fff', borderRadius:8, padding:18 }}>
          ...
          <View style={{ flexDirection:'row', alignItems:'center', marginBottom:12 }}>
            <TextInput value={path} onChangeText={setPath} placeholder="Đường dẫn tệp" style={{ flex:1, borderWidth:1, borderColor:'#ddd', padding:8, borderRadius:4, marginRight:8 }} />
            <TouchableOpacity onPress={browse} style={{ width:44, height:44, backgroundColor:'#eee', borderRadius:4, alignItems:'center', justifyContent:'center' }}>
              <Text>...</Text>
            </TouchableOpacity>
          </View>
          <View style={{ height:1, backgroundColor:'#eee', marginVertical:8 }} />
          <View style={{ flexDirection:'row', justifyContent:'flex-end' }}>
            <TouchableOpacity onPress={handleExport} style={{ paddingVertical:10, paddingHorizontal:14, marginRight:8, backgroundColor:'#e5e7eb', borderRadius:6 }}>
              <Text>Tải mẫu</Text>
            </TouchableOpacity>
            <TouchableOpacity onPress={handleConfirm} style={{ paddingVertical:10, paddingHorizontal:14, marginRight:8, backgroundColor:'#60a5fa', borderRadius:6 }}>
              <Text style={{ color:'#fff' }}>Đồng ý</Text>
            </TouchableOpacity>
            <TouchableOpacity onPress={() => { setPath(''); setCandidateUri(null); onClose(); }} style={{ paddingVertical:10, paddingHorizontal:14, backgroundColor:'#ef4444', borderRadius:6 }}>
